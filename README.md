...
Run every command below from the repo root (the folder containing `src/`, `notebook/`, `data/`, `assets/`, `output/`):

```bash
pip install pandas numpy faker matplotlib seaborn scipy statsmodels reportlab jupyter anthropic

python3 src/generate_data.py          # creates data/hr_bonus_dataset.csv
python3 src/bonus_engine.py           # creates data/hr_bonus_dataset_with_bonus.csv
jupyter nbconvert --to notebook --execute --inplace notebook/analysis.ipynb
jupyter nbconvert --to notebook --execute --inplace notebook/people_ops_agent_demo.ipynb
```

Or open `notebook/analysis.ipynb` / `notebook/people_ops_agent_demo.ipynb` directly in Jupyter/VS Code/Google Colab and run cell by cell — both notebooks resolve the `data/` and `assets/` paths automatically based on their own location, so this works whether Jupyter is launched from the repo root or from inside `notebook/`.

To run the agent directly from the command line instead of the notebook demo:

```bash
python3 src/people_ops_agent.py
```
